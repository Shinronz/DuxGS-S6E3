...
int CGMMng::ManagementProc(LPOBJ lpObj, char* szCmd, int aIndex) //00570A00
{
	...
	case COMMAND_RESETPK://identical
		{
			...
			TNotice pNotice(1);
			pNotice.SendToUser(lpTargetObj->m_Index, "PK Reset | Setting Normal Pk user");

			lpTargetObj->m_PK_Count = 0;
			lpTargetObj->m_PK_Level = 3;
			lpTargetObj->m_PK_Time = 0;

			if(lpTargetObj->PartyNumber >= 0)
			{
				gParty.UpdatePKUserInfo(lpTargetObj->PartyNumber, lpTargetObj->m_Index, lpTargetObj->DBNumber, lpTargetObj->m_PK_Level);
				gParty.UpdatePKPartyPanalty(lpTargetObj->PartyNumber);
			}
		}
		break;
	...
	case COMMAND_NOTICEALL://Identical
		{
			TNotice pNotice(0);
			pNotice.SendToAllUser("[%s] %s",lpObj->Name,this->GetTokenString());//this->GetTokenString()
		}
		break;
	case COMMAND_SETPKLEVEL://Identical
		{
			...
			if(iTokenNumber == 1)
			{
				lpTargetObj->m_PK_Level = 1;
			}
			else if(iTokenNumber == 2)
			{
				lpTargetObj->m_PK_Level = 2;
			}
			...
			{
				lpTargetObj->m_PK_Level = 4;
				lpTargetObj->m_PK_Count = 1;
			}
			else if(iTokenNumber == 5)
			{
				lpTargetObj->m_PK_Level = 5;
				lpTargetObj->m_PK_Count = 2;
			}
			else if(iTokenNumber == 6)
			{
				lpTargetObj->m_PK_Level = 6;
				lpTargetObj->m_PK_Count = 3;
			}
			else
			{
				lpTargetObj->m_PK_Level = 3;
			...
			{
				GCServerMsgStringSend(lMsg.Get(1137), lpTargetObj->m_Index, 1);
			}
	...
	case COMMAND_SETMONEY:
		{
			...
			int pIndex = lpTargetObj->m_Index;

			gObj[pIndex].Money = iMoney;
			GCMoneySend(pIndex, gObj[pIndex].Money);

			LogAddTD("Use GM Command -> [ %s ][ %s ][ %s ] / Target : [%s][%s] Zen : [%d] : %s",
				lpObj->Ip_addr, lpObj->AccountID, lpObj->Name, lpTargetObj->AccountID, lpTargetObj->Name, iMoney, "SetMoney");
		}
		break;
	...
	case COMMAND_SETDEFSTATS:
		{
			...
			int iClass = lpTargetObj->Class;

			lpTargetObj->ChangeUP = 0;

			lpTargetObj->Strength = DCInfo.DefClass[iClass].Strength;
			lpTargetObj->Dexterity = DCInfo.DefClass[iClass].Dexterity;
			lpTargetObj->Vitality = DCInfo.DefClass[iClass].Vitality;
			lpTargetObj->Energy = DCInfo.DefClass[iClass].Energy;
			lpTargetObj->Life = DCInfo.DefClass[iClass].Life;
			lpTargetObj->MaxLife = DCInfo.DefClass[iClass].MaxLife;
			lpTargetObj->Mana = DCInfo.DefClass[iClass].Mana;
			lpTargetObj->MaxMana = DCInfo.DefClass[iClass].MaxMana;
			lpTargetObj->Leadership = DCInfo.DefClass[iClass].Leadership;

			if(lpTargetObj->Class == CLASS_DARKLORD || lpTargetObj->Class == CLASS_MAGUMSA)
			{
				lpTargetObj->LevelUpPoint = 7 * (lpTargetObj->Level -1 );
			}
			else
			{
				lpTargetObj->LevelUpPoint = 5 * (lpTargetObj->Level -1 );
			}
	...
	case COMMAND_CLEARINVEN:
		...
		this->CmdClearInven(lpObj,lpTargetObj);
		}
		break;
...
#pragma message("!!!!!!!!!!!!!!!!! ADD PREFIX SETTING !!!!!!!!!!!!!!!!")
				...
				MessageSendEx(g_PostChatColor, lpObj->Name, "[POST] %s", Token);
			}
			else
			{
				break;
	...
	case COMMAND_ONLINE:
		{
			MsgOutput(lpObj->m_Index, "Users Online: %d", gObjTotalUser);
		}
		break;
	case COMMAND_ADDCREDITS: 
		{
			pId = this->GetTokenString();
			int iTokenNumber = this->GetTokenNumber();
			int iTokenNumber1 = this->GetTokenNumber();
			...
			if(iTokenNumber < 1 || iTokenNumber > 3)
			{
				TNotice pNotice(1);
				pNotice.SendToUser(lpObj->m_Index, "No such type of credits");
				return 0;
			} 
			...
			case 1:
				{
					lpTargetObj->GameShop.WCoinP = lpTargetObj->GameShop.WCoinP + iTokenNumber1;
					aux = lpTargetObj->GameShop.WCoinP;
					gGameShop.GDSavePoint(lpTargetObj->m_Index);
					MsgOutput(lpObj->m_Index, "[%d] WCoinP added to Account [%s] Now Have [%d]", iTokenNumber1, lpTargetObj->AccountID, aux);
					LogAddTD("Use Add Credits Command [%d]WCoinP [%s]Account -> Success!!", iTokenNumber1, lpTargetObj->AccountID);
				} break;
			case 2:
				{
					lpTargetObj->GameShop.WCoinC = lpTargetObj->GameShop.WCoinC + iTokenNumber1;
					aux = lpTargetObj->GameShop.WCoinC;
					gGameShop.GDSavePoint(lpTargetObj->m_Index);
					MsgOutput(lpObj->m_Index, "[%d] WCoinC added to Account [%s] Now Have [%d]", iTokenNumber1, lpTargetObj->AccountID, aux);
					LogAddTD("Use Add Credits Command [%d]WCoinC [%s]Account -> Success!!", iTokenNumber1,lpTargetObj->AccountID);
				} break;
			case 3: 
				{
					lpTargetObj->GameShop.GoblinPoint = lpTargetObj->GameShop.GoblinPoint + iTokenNumber1;
					aux = lpTargetObj->GameShop.GoblinPoint;
					gGameShop.GDSavePoint(lpTargetObj->m_Index);
					MsgOutput(lpObj->m_Index, "[%d] GoblinPoint added to Account [%s] Now Have [%d]", iTokenNumber1, lpTargetObj->AccountID, aux);
					LogAddTD("Use Add Credits Command [%d]GoblinPoint [%s]Account -> Success!!", iTokenNumber1, lpTargetObj->AccountID);
				} break;
			}
		}
		break;

		case COMMAND_SETCREDITS: 
		{
			pId = this->GetTokenString();
			int iTokenNumber = this->GetTokenNumber();
			int iTokenNumber1 = this->GetTokenNumber();
			...
			else if(iTokenNumber1 > 500000)
			{
				iTokenNumber1 = 500000;
			}
			...
			case 1:
				{
					lpTargetObj->GameShop.WCoinP = iTokenNumber1;
					aux = lpTargetObj->GameShop.WCoinP;
					gGameShop.GDSavePoint(lpTargetObj->m_Index);
					MsgOutput(lpObj->m_Index, "[%d] WCoinP setted to Account [%s] Now Have [%d]", iTokenNumber1, lpTargetObj->AccountID, aux);
					LogAddTD("Use Set Credits Command [%d]WCoinP [%s]Account -> Success!!", iTokenNumber1, lpTargetObj->AccountID);
				} break;
			case 2:
				{
					lpTargetObj->GameShop.WCoinC = iTokenNumber1;
					aux = lpTargetObj->GameShop.WCoinC;
					gGameShop.GDSavePoint(lpTargetObj->m_Index);
					MsgOutput(lpObj->m_Index, "[%d] WCoinC setted to Account [%s] Now Have [%d]", iTokenNumber1, lpTargetObj->AccountID, aux);
					LogAddTD("Use Set Credits Command [%d]WCoinC [%s]Account -> Success!!", iTokenNumber1,lpTargetObj->AccountID);
				} break;
			case 3: 
				{
					lpTargetObj->GameShop.GoblinPoint = iTokenNumber1;
					aux = lpTargetObj->GameShop.GoblinPoint;
					gGameShop.GDSavePoint(lpTargetObj->m_Index);
					MsgOutput(lpObj->m_Index, "[%d] GoblinPoint setted to Account [%s] Now Have [%d]", iTokenNumber1, lpTargetObj->AccountID, aux);
					LogAddTD("Use Set Credits Command [%d]GoblinPoint [%s]Account -> Success!!", iTokenNumber1, lpTargetObj->AccountID);
				} break;
			}
		}
		break;
	case COMMAND_SETLEVEL:
		{
			...
			gObj[pIndex].Level = iTokenNumber;

			//lpObj->m_i64MasterLevelExp = g_MasterLevelSystem.GetMasterLevelExpTlbInfo(iTokenNumber);
			//lpObj->m_i64NextMasterLevelExp = g_MasterLevelSystem.GetMasterLevelExpTlbInfo(iTokenNumber+1);

			if (gObj[pIndex].m_Index == lpObj->m_Index)
			{
				MsgOutput(gObj[pIndex].m_Index, "Level Updated %d | Please Reconect", iTokenNumber);
			} 
			else
			{	
				MsgOutput(lpObj->m_Index, "[%s][%s] Level Updated %d",gObj[pIndex].AccountID, gObj[pIndex].Name, iTokenNumber);
				MsgOutput(gObj[pIndex].m_Index, "Level Updated %d | Please Reconect", iTokenNumber);
			}

			LogAddTD("Use Test Command [%s][%s] /setlevel %d -> Success!!",gObj[pIndex].AccountID, gObj[pIndex].Name, iTokenNumber);
		}
		break;
}
return 0;
...
void CGMMng::CmdClearInven(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	char szText[256];
	memset(szText,0,sizeof(szText));
	...
	{
		if(lpTargetObj->pInventory[i].IsItem() == 1)
		{
			gObjInventoryDeleteItem(lpTargetObj->m_Index, i);
			ItemCount++;
		}
	}
	...
	wsprintf(szText, "Result-ClearInven(%d) Char: %s", ItemCount, lpTargetObj->Name);
