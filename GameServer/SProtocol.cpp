		...
		if ( gObj[n].Connected >= PLAYER_LOGGED )
		{
			if ( gObj[n].AccountID[0] == szId[0] )
			{
				if ( gObj[n].AccountID[1] == szId[1] )
				{
					...
					{
						if (gObj[n].LoginMsgCount >= 2)
						{
							gObjUserKill(n);
							g_ConnectEx.SendClose(n);
							LogAddTD("Usuario Desconectado por [%d] ingresos al estar conectado", gObj[n].LoginMsgCount);
						}
						gObj[n].LoginMsgCount ++;
